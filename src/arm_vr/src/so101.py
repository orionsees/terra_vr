...
class SO101Arm:
    ...
    def read_positions_with_raw(
        self, servo_ids: Optional[list[int]] = None
    ) -> tuple[dict[str, "float | int | None"], dict[int, Optional[int]]]:
        """Read positions in a single serial pass; returns (named_map, raw_map)."""
        raw_map = self.read_positions_raw(servo_ids)
        return self._format_positions(raw_map), raw_map
    ...
    def enable_torque(self, servo_ids: Optional[list[int]] = None) -> None:
        """Write 1 to register 40 (torque enable) for each servo.

        Call this before sending goal positions when torque may be off
        (e.g. right after power-on or after lerobot-calibrate exits).
        """
        if servo_ids is None:
            servo_ids = list(self._id_to_name.keys()) or list(range(1, 7))
        bus = self._bus_required()
        for sid in servo_ids:
            bus.write_data(sid, REG_TORQUE_ENABLE, b"\x01", expect_ack=False)
