#!/usr/bin/env python3
"""SO101 Bridge (Left Arm) — serial gateway using IPC bus instead of ROS2.

Commands received on bus port 5556 (JSON with "topic" and "data"):
...
Feedback published on bus port 5558:
  topic="left_arm/joint_states"  → positions, velocities, efforts, raw_positions, temperatures

Launch:
    python3 so101_bridge_left.py --module-dir <src_dir> --port /dev/ttyACM0
"""
...
from bus import CommandServer, Publisher, LEFT_ARM_CMD_PORT, LEFT_ARM_FEEDBACK_PORT
...
class SO101BridgeLeft:
    def __init__(self, args) -> None:
        ...
        self._cmd_srv = CommandServer(LEFT_ARM_CMD_PORT)
        self._fb_pub  = Publisher(LEFT_ARM_FEEDBACK_PORT)
        ...
        print(
            f"[left bridge] ready — port={args.port}  "
            f"cmd_port={LEFT_ARM_CMD_PORT}  "
            f"feedback_port={LEFT_ARM_FEEDBACK_PORT}  "
            f"hz={self._hz:.1f}"
        )
    ...
    def _cmd_loop(self) -> None:
        for msg in self._cmd_srv.iter_commands():
            ...
            try:
                ...
                print(f"[left bridge] cmd error: {exc}")
        ...
        while self._running:
            if self._bus_lock.acquire(blocking=False):
                try:
                    ...
                    print(f"[left bridge] feedback error: {exc}")
                finally:
