...
def _load_saved_ports() -> dict:
    """Read ports.json written by setup_and_launch.py, if it exists."""
    # Try to find the config dir relative to this launch file
...
def generate_launch_description():
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                "right_arm_port",
                default_value=saved.get("right_arm_port", "/dev/ttyACM0"),
                description="Serial port connected to SO101 right arm",
            ),
            DeclareLaunchArgument(
                "left_arm_port",
                default_value=saved.get("left_arm_port", "/dev/ttyACM1"),
