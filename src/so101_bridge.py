...
class SO101Bridge(Node):
    def __init__(self, args):
        ...
        self.pub_joint_states = self.create_publisher(JointState, "/so101/joint_states", qos)
        self.pub_raw_positions = self.create_publisher(
            Int32MultiArray, "/so101/raw_positions", qos
        )
        self.pub_temp = self.create_publisher(
            Float64MultiArray, "/so101/temp", qos
        )

        self.create_subscription(
            String,
            "/so101/cmd/set_positions",
            self._on_set_positions_topic,
            qos,
        )
        self.create_subscription(
            String,
            "/so101/cmd/set_positions_raw",
            self._on_set_positions_raw_topic,
            qos,
        )

        self.create_service(AddTwoInts, "/so101/ping", self._on_ping)
        self.create_service(Trigger, "/so101/scan", self._on_scan)
        self.create_service(Trigger, "/so101/set_positions", self._on_set_positions_service)
        self.create_service(
            Trigger,
            "/so101/set_positions_raw",
